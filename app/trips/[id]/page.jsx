...
export default function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const BASE = process.env.NEXT_PUBLIC_API_URL || "https://abudabbba-backend.vercel.app/api";
  ...
  const totalEgp = useMemo(() => {
    if (!trip) return 0;
    return Number(trip?.prices?.adult?.egp || 0) * adult +
           Number(trip?.prices?.child?.egp || 0) * child;
  }, [trip, adult, child]);

  const totalEuro = useMemo(() => {
    if (!trip) return 0;
    return Number(trip?.prices?.adult?.euro || 0) * adult +
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-7 lg:col-span-8">
            ...
            <p className="max-w-3xl text-gray-800 leading-relaxed">{trip.description}</p>

            <h2 className="mt-8 text-2xl font-semibold text-gray-900">About this activity</h2>
            <div className="mt-4">
              {(trip.features ?? []).map((f, i) => (
                <div key={f?.title ? `${f.title}-${i}` : i} className="flex gap-4 py-4 border-b last:border-none">
                  <div>
                    <div className="font-semibold text-gray-900">{f?.title}</div>
                    {f?.subtitle && <div className="text-gray-600 mt-1 text-sm leading-relaxed">{f.subtitle}</div>}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="sticky top-6">
              <div className="relative rounded-3xl p-[1.5px] bg-gradient-to-br from-blue-300/70 via-yellow-500/30 to-blue-300/70 shadow-[0_12px_40px_-10px_rgba(234,179,8,0.45)]">
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-[#19377c] to-[#165bd3] text-zinc-100 px-6 py-6">
                  <div className="mb-4 text-blue-200 font-semibold tracking-wide uppercase">Enjoy Your Trip</div>
                    ...
                    <form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
                      ...
                      <div className="flex items-center justify-between rounded-3xl bg-white/5 backdrop-blur ring-1 ring-blue-300/30 px-4 py-3">
                        <div className="flex flex-col">
                          <label htmlFor="adult" className="text-[18px] font-semibold text-blue-300">Adult</label>
                          <p className="text-zinc-300">
                            Price : {trip?.prices?.adult?.euro ?? 0} $ - ({trip?.prices?.adult?.egp ?? 0} EGP)
                          </p>
                        </div>
                        <input
                      ...
                      <div className="flex items-center justify-between rounded-3xl bg-white/5 backdrop-blur ring-1 ring-blue-300/30 px-4 py-3">
                        <div className="flex flex-col">
                          <label htmlFor="child" className="text-[18px] font-semibold text-blue-300">Child</label>
                          <p className="text-zinc-300">
                            Price : {trip?.prices?.child?.euro ?? 0} $ - ({trip?.prices?.child?.egp ?? 0} EGP)
                          </p>
                        </div>
                        <input
                      ...
                      <div className="flex items-center justify-between gap-3 rounded-3xl bg-white/5 backdrop-blur ring-1 ring-blue-300/30 px-4 py-3">
                        <div className="flex flex-col">
                          <label htmlFor="date" className="text-[18px] font-semibold text-blue-300">Date</label>
                        </div>
                        <input
                      ...
                      <div className="flex items-center gap-3 px-2 py-1 rounded-3xl">
                        ...
                        />
                        <label htmlFor="transfer" className="text-[15px] font-bold text-white">
                          Required transfer
                        </label>
                      </div>
