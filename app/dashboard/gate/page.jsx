  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      if (typeof window !== "undefined") {
        ...
        if (!isSecure) {
          setError(
            "المتصفح منع الوصول للكاميرا لأن الاتصال غير مشفّر. افتح الصفحة عبر HTTPS أو على localhost."
          );
          setStatus("");
          return;
        }
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
          setError(
            "هذا المتصفح لا يدعم الكاميرا أو تم حظرها. جرّب متصفحًا آخر أو حدّثه."
          );
          setStatus("");
          return;
        }
      ...
      try {
        const { Html5QrcodeScanner, Html5QrcodeScanType } = await import(
          "html5-qrcode"
        );
        ...
        setError(
          `تعذر تشغيل الكاميرا (${reason}). تأكد من الأذونات والمتصفح وHTTPS.`
        );
      }
    }
