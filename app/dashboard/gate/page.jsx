...
export default function BookingScanner({ apiBase }) {
  ...
  async function fetchBooking(bid, refFromQR) {
    ...
    return {
      ...
      client: [b?.user?.firstName, b?.user?.lastName].filter(Boolean).join(' ') || '—',
      phone: b?.user?.phone || '—',
      ref: refFromQR || ref,
      date: b?.bookingDate ? new Date(b.bookingDate).toLocaleString('ar-EG') : '—',
      totalEgp: b?.totalPrice?.egp != null ? Number(b.totalPrice.egp).toFixed(2) : '—',
      payment: !!b?.payment,
    };
  }
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (!isSecure) {
          setError('المتصفح منع الوصول للكاميرا لأن الاتصال غير مشفّر. افتح الصفحة عبر HTTPS أو على localhost.');
          setStatus('');
          return;
        }
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
          setError('هذا المتصفح لا يدعم الكاميرا أو تم حظرها. جرّب متصفحًا آخر أو حدّثه.');
          setStatus('');
          return;
        }
      }
      try {
        ...
        const reason = e?.name || e?.message || 'unknown';
        setError(`تعذر تشغيل الكاميرا (${reason}). تأكد من الأذونات والمتصفح وHTTPS.`);
      }
    }
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gradient-to-b from-[#0b1320] to-[#111827] text-slate-200">
      <div className="max-w-5xl mx-auto p-4 md:p-6">
        <header className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-sky-500 flex items-center justify-center font-bold">QR</div>
          <div>
            <h1 className="text-xl font-semibold">ماسح الحجوزات</h1>
            <p className="text-slate-400 text-sm">وجّه الكاميرا نحو كود التذكرة لإظهار التفاصيل</p>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900/80 border border-white/10 rounded-2xl p-4 shadow-xl">
            ...
            <div className="text-2xl font-bold tracking-wide mt-1">{details.trip}</div>
        ...
        <footer className="text-center text-slate-400 text-xs mt-4">يُفضّل إضاءة جيدة ومسافة 15–25 سم من الكود.</footer>
      </div>
    </div>
  );
}
